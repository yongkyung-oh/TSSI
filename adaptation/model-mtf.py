...
if not os.path.exists('out_mc-mtf'):
    os.mkdir('out_mc-mtf')
    
for dataname in dataname_list:
    if not os.path.exists(os.path.join('out_mc-mtf', dataname)):
        os.mkdir(os.path.join('out_mc-mtf', dataname))
...
for dataname in dataname_list:
    for k in range(10):
        try:
            ...
            with open(os.path.join('out_mc-mtf', dataname, out_name), 'rb') as f:
                out = pickle.load(f)
        except:
...
for dataname in dataname_todo:
    ...
    def transform_function(sample, kwargs=kwargs):
        return transform_MTF(sample, **kwargs)
    ...
    for k in range(10):
        ...
        # check exists
        if os.path.exists(os.path.join('out_mc-mtf', dataname, '{}_{}_{}_{}'.format(model_name, str(transfer), str(sparse), k))):
            continue
        ...
        with open(os.path.join('out_mc-mtf', dataname, '{}_{}_{}_{}'.format(model_name, str(transfer), str(sparse), k)), 'wb') as f:
