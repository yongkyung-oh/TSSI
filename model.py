...
if not os.path.exists('out_mc'):
    os.mkdir('out_mc')
    
for dataname in dataname_list:
    if not os.path.exists(os.path.join('out_mc', dataname)):
        os.mkdir(os.path.join('out_mc', dataname))
...
for dataname in dataname_list:
    for k in range(10):
        try:
            ...
            with open(os.path.join('out_mc', dataname, out_name), 'rb') as f:
                out = pickle.load(f)
        except:
...
for dataname in dataname_todo:
    ...
    def get_resolution(s):
        if s < 256:
            ...
            return max([i for i in [256, 512, 1024] if i <= s])    
    
    # set size
    size = get_resolution(max_len) # 2**8 # default 256 * 256
    tensor_size = 224
    if num_dim > 100:
    ...
    def transform_function(sample, kwargs=kwargs):
        return transform_TSSI(sample, **kwargs)

    if sparse:
        image_transform = transforms.Compose([
                transforms.Resize((tensor_size, tensor_size)), # for CNN model 
                transforms.ToTensor(),
                # skip normalization for sparse representation
                ])
    else:
        image_transform = transforms.Compose([
                transforms.Resize((tensor_size, tensor_size)), # for CNN model 
    ...
    for k in range(10):
        ...
        # check exists
        if os.path.exists(os.path.join('out_mc', dataname, '{}_{}_{}_{}'.format(model_name, str(transfer), str(sparse), k))):
            continue
        ...
        with open(os.path.join('out_mc', dataname, '{}_{}_{}_{}'.format(model_name, str(transfer), str(sparse), k)), 'wb') as f:
