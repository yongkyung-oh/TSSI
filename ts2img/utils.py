...
def normalize(X_train, X_test):
    """
    A function that z-normalizes the input data for training and testing sets.
...
def get_np_format(X):
    """
    A function that transforms a pandas DataFrame to a numpy array format.
    ...
    for i in range(X.shape[1]):
        X_np.append(pd.concat(X.iloc[:,i].tolist(), axis=1).T.to_numpy())
    X_np = np.stack(X_np, axis=1)
    return X_np


def get_data(dataname, adjust_len=None):
    """
    Load time series data from the UCR/UEA repository or local files, and return it. 
    The function also provides the option to adjust the length of the time series to a predefined value using interpolation.

    Args:
        - dataname: The name of the dataset from the UCR/UEA repository or local files.
        - adjust_len: If specified, the length of the time series will be adjusted to this value using interpolation.
    """
    try:
    ...
    # adjust length of time series using interpolation
    def get_same_len(s, max_len):
        s_re = np.interp(
               np.arange(0,max_len),
    ...
    if min(len_list) == max(len_list):
        if adjust_len:
            X_train = X_train.applymap(lambda s: get_same_len(s, max_len=adjust_len))
            X_test = X_test.applymap(lambda s: get_same_len(s, max_len=adjust_len))
        else:
            pass
    else:
        if adjust_len:
        ...
        X_train = X_train.applymap(lambda s: get_same_len(s, max_len=max_len))
        X_test = X_test.applymap(lambda s: get_same_len(s, max_len=max_len))
